...
def main(batch_size, num_workers, epochs, load_weights=False):
    name = "eurlex43k"
    train_loader, test_loader, info = load_dataset(train_file=f"../data/EURLex-4.3K/train.txt",
                                                   test_file=f"../data/EURLex-4.3K/test.txt",
    ...
    network = Network(device, in_features=info["features"], hidden=512, out_features=400, labels=info["labels"],
                      requires_grad=False, negative=True, factor=1, drop_rate=0.0)
    ...
    ----------Tests with Ordered Retrieval------------
                      1       2       3       4       5
    -----------  ------  ------  ------  ------  ------
    Precision@k  88.204  83.998  78.299  71.008  63.853
    nDCG@k       88.204  85.183  82.034  79.237  77.298
    PSprec@k     43.622  50.605  54.797  56.864  58.021
    PSnDCG@k     43.622  48.873  52.066  53.943  55.080
    All Done!
