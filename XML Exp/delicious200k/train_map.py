...
from network_spmap import Network
...
def main(batch_size, num_workers, epochs, load_weights=False):
    name = "delicious200k"
    train_loader, test_loader, info = load_dataset(train_file=f"../data/DeliciousLarge/deliciousLarge_train.txt",
    ...
    network = Network(device, in_features=info["features"], hidden=1024, out_features=3000, labels=info["labels"],
                      requires_grad=False, negative=True, factor=1, drop_rate=0.25, reduce_dim=True, kernel_dim=7)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for data in (pbar := tqdm(train_loader)):
            ...
            pbar.set_description(f"Training loss: {torch.mean(torch.tensor(logger['loss'])):>6.4f}")
        ...
        with torch.no_grad():
            ...
            for data in (pbar := tqdm(test_loader)):
                ...
                # evaluate
                y_pred = network.inference(y_logits, steps=10)
                logger["loss"].append(loss)
                ...
                pbar.set_description(f"Testing loss: {torch.mean(torch.tensor(logger['loss'])):>6.4f}")
    ...
    ----------Tests with Ordered Retrieval------------
                      1       2       3       4       5
    -----------  ------  ------  ------  ------  ------
    Precision@k  45.373  42.044  39.897  38.340  37.044
    nDCG@k       45.373  42.829  41.184  39.984  38.992
    PSprec@k      6.862   7.353   7.776   8.160   8.494
    PSnDCG@k      6.862   7.209   7.489   7.731   7.933
    All Done!
...
if __name__ == '__main__':
    ...
    main(batch_size=64, num_workers=4, epochs=25, load_weights=False)
