...
_, test_loader = mnist(root=root(), batch_size=128, num_workers=4)
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
_, vtb_mod = vtb(batch_size=128, input_dim=28, device=device)
print(device)
...
with torch.no_grad():
    ...
    for data in test_loader:
        ...
        if method == "vtb":
            key = normal(x_true.shape, std=1. / sqrt(28. * 28. * 1.), ).to(device)
            x_true = normalize(vtb_mod.binding(x_true, key, ch=1))
            ...
            key = normal_mixture(x_true.shape, std=1. / sqrt(28. * 28. * 1.)).to(device)
            x_true = normalize(x_true * key)
