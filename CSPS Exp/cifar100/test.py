...
_, test_loader = cifar100_augmented(root=root(), batch_size=128, num_workers=4)
...
with torch.no_grad():
    ...
    print("top 5 accuracy:", torch.mean(torch.tensor(logger["acc@5"])))
...
top5acc = evaluate_top(torch.tensor(y_true), torch.tensor(y_pred), top=5)
print(top5acc)
