...
train_loader, test_loader = cifar10_augmented(root=root(), batch_size=64, num_workers=4)
device = torch.device("cuda:2" if torch.cuda.is_available() else "cpu")
print(device)

network = Network(in_channels=3, classes=10)
network.to(device)
# network.load_state_dict(torch.load('./../weights/cifar10_map.h5'))
...
torch.save(network.state_dict(), './../weights/cifar10_map.h5')
print('All Done!')
